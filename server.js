...
app.post('/login', async (req, res) => {
  try {
    ...
    // search the database for a user with the proviced username
    const user = await User.findOne({username})
    // if no user is found, return error
    if(!user){
      throw new Error("No user with that username found")
    }
    // if a user if found, let's compare the provided password to the the password on the user database object
    const passwordCheck = await bcrypt.compare(password, user.password)
    // if doesn't match, throw error
    if(!passwordCheck){
      throw new Error('Password does not match')
    }
    // create a token with the username in the payload jwt.sign means create a token
